...
import { Eye, RotateCcw, Stethoscope, StickyNote, Video } from 'lucide-react'
import { getSessionDetail } from '../data/sessionData'
import { formatMoney, formatShortUuid, sessionStatusClass, sessionStatusLabel } from '../lib/display'
...
export default function SessionDetail() {
  ...
  const canRefund = d.overviewStatus === 'confirmed'
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="text-xs font-semibold text-[var(--figma-text-muted)]">
            ...
            <span className="text-[var(--figma-text-strong)]">#{formatShortUuid(d.id)}</span>
          </div>
          ...
          <div className="figma-card overflow-hidden p-5 sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-[var(--figma-text-muted)]">Primary session metadata and video room details.</p>
              </div>
              <span
                className={['inline-flex rounded-full px-3 py-1 text-[11px] font-semibold', sessionStatusClass(d.overviewStatus)].join(' ')}
              >
                {sessionStatusLabel(d.overviewStatus).toUpperCase()}
              </span>
            </div>
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
              <div>
                ...
                <div className="mt-1 text-sm font-semibold text-[var(--figma-text-strong)]">{formatShortUuid(d.id)}</div>
              </div>
              <div>
              ...
              <div className="flex flex-wrap items-start gap-3">
                ...
                <div className="min-w-0 flex-1 space-y-2 text-xs text-[var(--figma-text-muted)]">
                  ...
                  <div>
                    ...
                    <span className="font-mono text-[var(--figma-text)]">{d.video.roomId}</span>
                  </div>
                  <div>
                    <span className="font-semibold">Join window:</span> {d.video.joinWindowStart} – {d.video.joinWindowEnd}
                  </div>
                  <div>
                    <span className="font-semibold">Healer joined:</span> {d.video.healerJoinedAt ?? '—'}
                  </div>
                  <div>
                    <span className="font-semibold">Client joined:</span> {d.video.clientJoinedAt ?? '—'}
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="mt-3 flex items-start justify-between gap-2">
                  ...
                  {d.client.id ? (
                    <button
                      ...
                      onClick={() => navigate(`/clients/${encodeURIComponent(d.client.id)}`)}
                    >
                ...
                <div className="mt-3 flex items-start justify-between gap-2">
                  ...
                  {d.practitioner.id ? (
                    <button
                      ...
                      onClick={() => navigate(`/practitioners/${encodeURIComponent(d.practitioner.id)}`)}
                    >
                      <Eye className="h-4 w-4" />
                    </button>
                  ) : null}
          ...
          <div className="overflow-hidden rounded-[14px] bg-[var(--figma-brand)] p-6 text-white shadow-[0_12px_32px_rgba(27,20,100,0.18)]">
            ...
            <div className="mt-5 space-y-3 text-sm">
              <div className="flex justify-between gap-2 border-b border-white/15 pb-2">
                <span className="text-white/80">Session earning</span>
                <span className="font-semibold">{formatMoney(fin.sessionEarning)}</span>
              </div>
              <div className="flex justify-between gap-2 border-b border-white/15 pb-2">
                <span className="text-white/80">Commission ({Math.round(fin.commissionRate * 100)}%)</span>
                <span className="font-semibold">{formatMoney(fin.commissionFee)}</span>
              </div>
              <div className="flex justify-between gap-2 border-b border-white/15 pb-2">
                <span className="text-white/80">Platform fee</span>
                <span className="font-semibold">{formatMoney(fin.platformFee)}</span>
              </div>
            </div>
            <div className="mt-5 rounded-[12px] bg-white/10 p-4">
              <div className="text-xs text-white/75">Healer net</div>
              <div className="mt-1 text-2xl font-semibold tracking-tight">{formatMoney(fin.healerNet)}</div>
              <div className="mt-2 text-xs text-white/75">
                Wallet: {fin.walletState === 'available' ? 'Available' : 'Pending'}
              </div>
            </div>
          </div>
